# -*- coding: utf-8 -*-
"""
Created on Fri Nov  4 21:56:34 2022
...
for d in range(0,360,10):
    for g in range(0,360,10):
        for m in range(0,360,10):
            #m is phi
            ph=m
            for r in range(0,181,5): #n is theta
                th=r
                ...
                e2=np.reshape([1+np.cos(np.deg2rad(r+g+d)),np.sin(np.deg2rad(r+g+d))*(np.cos(np.deg2rad(ph))-j*np.sin(np.deg2rad(ph))),np.sin(np.deg2rad(r+g+d))*(np.cos(np.deg2rad(ph))+j*np.sin(np.deg2rad(ph))),1-np.cos(np.deg2rad(r+g+d))],(2,2))/2
                e3=np.eye(2)-e0-e1-e2
                w1 = np.linalg.eigvals(e0)
                w2 = np.linalg.eigvals(e1)
                w3 = np.linalg.eigvals(e2)
                w4 = np.linalg.eigvals(e3)
                
                for z in range(2):
                    if w1[z]>=0 and w2[z]>=0 and w3[z]>=0 and w4[z]>=0 :
                        proceed=True
                    else:
                        proceed=False
                        #print(m,r)
